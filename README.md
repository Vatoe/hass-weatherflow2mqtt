...
- **Fixed the High/Low table crash at its root.** `updateHighLow()` compared a new sensor reading against a stored high/low value without guarding against that stored value being `NULL` in the database (e.g. for a freshly-added sensor), which threw a `TypeError` that silently aborted high/low **and "latest" value** updates for every sensor on that report cycle — not just the affected one. Self-healing; no manual database changes needed.
- **Isolated per-sensor failures.** The fix above only covered the one known trigger — the surrounding `try`/`except` still wrapped the *entire* update loop, so any other single-sensor problem could still take down every other sensor's update for that cycle. Now each sensor is handled independently.
- **Parameterized the SQL that handles live sensor values** (`updateHighLow()`, `writePressure()`) instead of interpolating them into raw SQL text, with an explicit guard rejecting NaN/Infinity readings (logged, not silently written) rather than either crashing or corrupting the table.
- **Bumped stale dependencies**: `paho-mqtt` 1.6.1→2.1.0, `aiohttp` 3.8.4→3.14.1, `PyYAML`, `pytz`, and `pyweatherflowudp` (the actual UDP-parsing library — still independently maintained even though this add-on isn't) 1.4.2→1.5.2. Also bumped the Dockerfile's base image (`python:3.11-slim-buster` → `slim-bookworm`) since Debian 10's package repos are no longer served, which was blocking builds entirely.
- **Added a targeted test suite** (`tests/`) covering the fixes above, plus Dependabot so dependency staleness gets caught automatically going forward instead of requiring another manual audit.

### Why keep using this instead of Home Assistant Core's built-in WeatherFlow integration?

HA Core does have its own official `weatherflow` local-push integration now (that's *why* this add-on was archived — see the announcement below). It's actively maintained, but it only exposes the raw fields the Tempest station broadcasts. This add-on's real value is the computed-sensor layer sitting on top of that raw feed, which Core has no equivalent for: Zambretti pressure forecast (number + text), dewpoint/temperature/UV comfort-level categorizations, fog/snow probability, cloud base and freezing-level altitude, sea-level pressure (derived from station pressure + elevation), rain duration/intensity/totals, windowed lightning-strike counts, and more. If you only need raw numbers, Core's integration is simpler and has one less moving part (no MQTT hop). If you want the richer weather-analytics layer, this is still the tool for that job — now with an actual bug fix and someone paying attention to it.
